...
- **Evidence — 2026-07-11.** The "fast" suite (`-m "not slow"`) takes ~1.5 h both locally and on CI. Measured with `--durations`: the top offenders are all unmarked tests in `tests/test_split_plot.py` — `TestSplitPlotAnalysis::test_r2_mode_works` 1039 s, `TestCR25HtcColMapping::test_wp_contrast_auto_equals_conservative` 695 s, `test_sp_contrast_auto_equals_sp_only` 448 s, plus seven more at 170–240 s, plus a second tier of 14 at 90–150 s.
- **Partial — 2026-07-11.** All 23 measured heavy tests (>90 s, each annotated with its measured duration) are now `@pytest.mark.slow`; one ~130 s canary (`TestSplitPlotAPI::test_design_df_row_count_is_n_wp_times_s`) stays in the fast suite so the end-to-end split-plot path keeps CI coverage. `pytest tests/test_split_plot.py -m "not slow"`: 55+ min → 3 min 9 s (190 passed). Remaining for this ticket: the unit/integration directory reorg, and a class-scoped fixture for `TestSplitPlotAPI`/`TestSplitPlotAnalysis` — their tests each re-run an identical `find_optimal_design` call (~130 s apiece) and assert different properties of the same result; sharing one result via a fixture would let the slow-marked structure tests rejoin the fast suite at no extra cost.
- **Priority:** P0b
- **Release target:** v0.1.1
- **Depends on:** none
...
- **Status:** Done — 2026-07-10. Chose resolution 1 (cap + warn). All three fixed-candidate bisection sites in `api.py` (single-response incl. blocked, multi-response shared-formula/Hotelling, multi-response compound) now cap the search's upper bound at `min(max_n, n_ceiling)` via a shared `_capped_search_max_n` helper, where `n_ceiling` is the post-constraint candidate count (`n_blocks × n_cand` for blocked designs, which draw per block). Split-plot paths were audited and are immune (candidates are built per evaluation, sized to the request). A pool smaller than the minimum estimable design (`n_cand < p+1`, e.g. after aggressive constraint filtering) raises an actionable ValueError up front naming `candidate_points`/`cand_max`/`constraint_expr`. When the cap binds AND the target power is not reached, a RuntimeWarning naming both `max_n` and `candidate_points` is emitted and recorded in `report["warnings"]`. The mid-search candidate-growth path was additionally guarded to reject a rebuilt pool smaller than the original (would have invalidated the cap). Regression tests in [tests/test_search_bounds.py](tests/test_search_bounds.py) (9 tests, fast suite) cover the original crash, blocked, both MR paths, the cap warning, and both error paths. `max_n=50` workaround removed from the README quick-start and `tests/test_readme_smoke.py` (both verified passing; note the smoke test now takes ~60 s because the search bisects from the true `min(max_n=2000, n_cand=1000)` bound — bisection-bound tuning was explicitly out of scope). Docs: `max_n` ↔ `candidate_points` relationship documented in all three `Power*Config.max_n` docstrings, `DesignOptions.candidate_points`, user-guide §3.2, and user-guide §17.4 (new "Consequence 3"). Typed `CandidateExhaustionError` deferred to TICKET-008 as planned.
- **Priority:** P0b
- **Release target:** v0.1.1
