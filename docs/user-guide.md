...
**Multi-response results additionally carry `result["model_matrices"]`** — one matrix per response, in configured order. For ordinary multi-response runs these all equal `model_matrix`; in **compound** mode (per-response formulas) each response's power was computed on its *own* formula's matrix over the shared design rows, so `model_matrices[name]` — not the global `model_matrix` — is that response's authority. The data-dependent-coding check runs over every response formula, and the warning names the affected responses. The CLI writes one `_model_matrix_<response>.csv` per response for compound runs (free-form response names are slugged for filenames; `report["model_matrix_files"]` maps original names to files), the app offers per-response downloads, the REST multi-response payload includes `model_matrices`, and the Excel/Google Sheets connectors write one `MM_<slug>` sheet per response plus a `ModelMatrixIndex` sheet mapping original names to sheet titles. Re-exporting into the same workbook or spreadsheet **replaces the sheets the previous export generated**, so output sheets describe the current result even if response names changed only by case (which sheet titles do not distinguish). Ownership is verified independently of sheet content — a custom document property in Excel, sheet-scoped developer metadata in Google Sheets — never from the index cells, which are ordinary editable content: your own sheets are not deleted even if an edited index names them (or lists their IDs), and new titles dodge them case-insensitively. The fixed names `Results`, `Design`, `Buckets`, `ModelMatrix` and `ModelMatrixIndex` are reserved output titles that every export overwrites. One upgrade caveat: matrix sheets written by versions that predate ownership tracking can never be verified as Lattice DOE output, so the first export after upgrading leaves them in place permanently (they may hold outdated matrices) and emits a `RuntimeWarning` naming them — delete them manually if unwanted. On the app's Analysis page, a multi-response run shows a response selector, and sensitivity/MDE bind to the selected response's formula, power configuration and matrix.
...
| `clear_results` | `True` | Clear existing content in the ordinary result sheets (Results, Design, Buckets) before writing. The generated matrix sheets (`ModelMatrix`, `MM_*`, `ModelMatrixIndex`) are always fully cleared and replaced regardless — the API updates only the written rectangle, so stale rows from a larger previous matrix would otherwise contaminate a smaller new one. |
...
Every interface in this package calls the same underlying engine and supports the same configuration parameters. The choice between them is about workflow fit, not capability. This appendix maps each feature to each interface so you can pick the right tool for your situation and know in advance what you will not find there.

---

### C.1 Feature matrix

✓ = supported. — = not available in this interface.
...
The Excel interface uses a three-sheet workbook template (`SETTINGS`, `FACTORS`, `RESPONSES`). A Python call to `run_from_excel` reads the template, runs the design, and writes the result design table and summary metrics back into a new sheet in the same workbook. Generate a blank template with `write_excel_template`.

Excel supports all design types but not the post-design analysis functions. There is no report output — results are written directly to the workbook. The file must not be open in Excel when `run_from_excel` is called on Windows.

Best for: teams with an Excel-first workflow, stakeholders who want to specify factors and review results in a familiar tool.
...
The Sheets interface is structurally identical to the Excel interface: the same three-tab layout (`SETTINGS`, `FACTORS`, `RESPONSES`), same parameter set, same result write-back. The difference is that the workbook lives in Google Drive, so multiple collaborators can fill in the template simultaneously and access results without file transfers.

Authentication uses either a service account JSON key (automated/CI use) or OAuth2 browser flow (interactive use). The spreadsheet must be shared with the service account's email address.

Best for: distributed teams, organisations that use Google Workspace, collaborative design specification.
...
The Widgets interface renders an interactive form inside a Jupyter notebook cell using `ipywidgets`. It supports contrast mode, R² mode, and power-curve-by-n analysis. GLM mode, multi-response, split-plot, blocking, constraints, sensitivity analysis, MDE, and criteria comparison are not available through the widget form — use the Python API directly for those.

The widget is useful for users who work primarily in notebooks and want a guided form rather than typing configuration objects by hand. The form state can be extracted as a `DesignOptions` / power-config pair and passed on to the Python API for further processing.

Best for: notebook-based workflows, teaching, exploratory sessions where a form is more convenient than code.

Chapter references: §15 (Widgets).

#### REST API

The REST API exposes the package's core functions as HTTP endpoints at a running `uvicorn` server. All endpoints accept JSON bodies and return JSON responses. Clients in R, JavaScript, Julia, or any language with an HTTP library can use the API without a Python environment.
...
| `/mde` | POST | `min_detectable_effect` |
| `/compare_criteria` | POST | `compare_criteria` |
| `/augment` | POST | `augment_design` |
| `/health` | GET | Server health check |

The API does not generate HTML or PDF reports; report generation is a server-side file-system operation and is intentionally excluded from the HTTP interface. The `workers` field in `DesignOptions` is silently ignored inside an ASGI process — use `UVICORN_WORKERS` to scale server concurrency instead.

Best for: integration with non-Python platforms, shared design services, microservice architectures.
...
### C.3 Quick decision guide

| Situation | Recommended interface |
|---|---|
| I write Python and want full control | Python API |
| I need reproducible, version-controlled pipelines | CLI + YAML |
| My collaborators do not write code | Streamlit |
| My team works in Excel | Excel |
| My team works in Google Sheets or is distributed | Google Sheets |
| I want an interactive form inside a Jupyter notebook | Widgets |
| I am integrating with R, JavaScript, or another language | REST API |
| I need power curves or sensitivity analysis without Python | Streamlit or REST API |
| I need GLM mode without Python | Streamlit, Excel, Sheets, or REST API |
| I need split-plot or blocking without Python | Streamlit, Excel, Sheets, or REST API |
