...
| # | Enhancement | Description | Est. LOE | Value | Key files |
|---|---|---|---|---|---|
| ~~18~~ | ~~**Jupyter ipywidgets UI**~~ | ~~Interactive in-notebook UI: dynamic factor-entry table, sliders for `alpha`/`power`/`sigma`, formula text field, run button, and inline Plotly power curve. No server needed — runs inside any JupyterLab / VS Code notebook. Ideal for data scientists who prototype in notebooks.~~ | **Done.** `iopt_power_design/widgets.py` (new, ~500 lines): soft-dep guard for `ipywidgets`/`plotly`; `WidgetsError` exception; pure-Python helpers `_parse_matrix`, `_parse_vector`, `_build_power_cfg_from_state`, `_build_design_opts_from_state`; `_FactorRow` bundle class; `DesignWidget` class with dynamic factor table, power-mode toggle (R²/Contrast), shared power params, advanced accordion (criterion, starts, seed, candidate sizing, constraint expr), run button, inline Plotly power curve. Non-exposed `DesignOptions` fields (e.g. `n_blocks`) forwarded transparently via `_extra_do_kwargs`. `design_widget()` factory creates and displays in one call. `pyproject.toml` gains `[widgets]` extras (`ipywidgets>=8.0`, `plotly>=5.0`); `ipywidgets>=8.0` added to `[all]`. Public names exported from `__init__.py`. 47 new tests in `tests/test_widgets.py` across 4 layers (import guard, pure helpers, widget construction/interaction, run callback mock); all passing. | ~~3–4 days~~ | ~~Medium-High~~ | `iopt_power_design/widgets.py` (new), `iopt_power_design/__init__.py`, `pyproject.toml`, `tests/test_widgets.py` (new) |
