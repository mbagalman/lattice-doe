...
| CR-19 | `iopt_power_design/iopt_search.py` | 464–472 | **`xtx_jitter` is not passed into `_optimal_indices_from_X` in parallel worker path.** `_one_start_worker()` forwards `jitter` only to final score calculation, not to the optimization routine itself. | Parallel and serial searches can produce different designs for the same seed/settings when jitter differs from default, undermining reproducibility and numerical tuning. |
