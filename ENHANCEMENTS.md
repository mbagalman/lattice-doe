...
| CR-18 | `iopt_power_design/api.py` | 199–223 | **Blocked designs can fail for valid formulas with categorical treatment factors.** `p_full` is estimated from sample rows that pin treatment categoricals to one level, then compared against `p_treat` from full candidates. | False negatives (`ValueError: Blocked model has no block dummy columns`) block legitimate blocked runs whenever treatment terms expand with categorical levels. |
