...
| CR-23 | `app/pages/4_Analysis.py` | 225–237 | **Streamlit YAML export drops newer design knobs.** `_build_yaml()` serializes only a small subset of design fields and omits blocked/pre-allocation keys. | Exported YAML cannot round-trip full app intent into CLI/API runs, causing silent config loss and inconsistent behavior across interfaces. |
