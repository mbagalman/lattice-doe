...
class TestEstimateCandidateSize:
    def test_continuous_within_bounds(self):
        size = estimate_candidate_size("~ 1 + A + B", CONTINUOUS, cand_min=100, cand_max=5000)
        assert 100 <= size <= 5000

    def test_categorical_at_least_min(self):
        size = estimate_candidate_size("~ 1 + X", CATEGORICAL, cand_min=100, cand_max=5000)
        assert 100 <= size <= 5000

    def test_mixed_within_bounds(self):
        size = estimate_candidate_size("~ 1 + A + X", MIXED, cand_min=100, cand_max=5000)
        assert 100 <= size <= 5000

    def test_respects_cand_max(self):
        size = estimate_candidate_size("~ 1 + A", CONTINUOUS, cand_min=10, cand_max=50)
        assert size <= 50

    def test_respects_cand_min(self):
        size = estimate_candidate_size("~ 1 + X", {"Z": ["a", "b"]}, cand_min=500, cand_max=5000)
        assert size >= 500
...
class TestAugmentDesign:
    ...
    def seed_design(self):
        ...
        return pd.DataFrame({"A": [0.0, 0.5, 1.0, 0.25, 0.75], "B": [0.0, 0.5, 1.0, 0.75, 0.25]})
    ...
    def test_existing_rows_preserved(self, seed_design):
        ...
        pd.testing.assert_frame_equal(original_part, seed_design.reset_index(drop=True))

    def test_raises_on_zero_m(self, seed_design):
        with pytest.raises(ValueError, match="m"):
            augment_design(seed_design, m=0, formula=self.FORMULA, factors=self.FACTORS)

    def test_raises_on_empty_design(self):
        empty = pd.DataFrame({"A": [], "B": []})
        with pytest.raises(ValueError, match="empty"):
            augment_design(empty, m=2, formula=self.FORMULA, factors=self.FACTORS)
    ...
    def test_a_criterion_augmentation(self, seed_design):
        ...
        opts_a = DesignOptions(
            candidate_points=80, starts=1, max_iter=20, random_state=0, criterion="A"
        )
        aug, new = augment_design(
            seed_design,
            m=2,
...
class TestSR12CompoundDGain:
    ...
    def test_convergence_is_max_iter_parity_independent(self):
        ...
        i_a = _compound_fedorov_single(cands, weights, 10, criterion="D", max_iter=200, seed=7)
        i_b = _compound_fedorov_single(cands, weights, 10, criterion="D", max_iter=201, seed=7)
    ...
    def test_compound_d_beats_random_designs(self):
        ...
        cands, weights, rng = self._fixture()
        idx = build_compound_design(cands, weights, 10, criterion="D", n_start=3, random_state=1)
    ...
    def test_compound_i_regression(self):
        ...
        cands, weights, _ = self._fixture()
        idx = build_compound_design(cands, weights, 10, criterion="I", n_start=3, random_state=1)
...
class TestSR13CellCoverage:
    ...
    def test_small_budget_keeps_all_levels_and_warns(self):
        ...
        assert any("fewer than 2 continuous samples" in str(c.message) for c in caught)
    ...
    def test_no_permanently_unestimable_columns(self):
        ...
        assert all(np.abs(X[:, j]).max() > 0 for j in dummy_cols), (
            "a level's dummy column is all-zero — that treatment is " "permanently unestimable"
        )
    ...
    def test_cap_branch_repairs_level_coverage(self):
        ...
        factors = {"F1": ["1a", "1b", "1c"], "F2": ["2a", "2b", "2c"], "F3": ["3a", "3b", "3c"]}
        cand = build_candidate(factors, candidate_points=6, seed=0, cat_cells_cap=5)
        for f in factors:
    ...
    def test_pure_continuous_unchanged(self):
        ...
        cand = build_candidate({"x": (0.0, 1.0), "y": (-1.0, 1.0)}, candidate_points=50, seed=0)
        assert len(cand) == 50
...
class TestSR21ConditionNumberConvention:
    ...
    def test_condition_number_is_kappa_of_X(self):
        ...
        assert m["condition_number"] == pytest.approx(float(np.linalg.cond(X)), rel=1e-10)
        # And explicitly NOT the squared convention.
        assert m["condition_number"] != pytest.approx(float(np.linalg.cond(X.T @ X)), rel=1e-3)
...
class TestUX1ModelMatrixPreview:
    ...
    def test_categorical_interaction(self):
        ...
        p, _ = model_matrix_preview("~ a * b", {"a": ["x", "y"], "b": ["u", "v", "w"]})
        assert p == 6  # 1 + 1 + 2 + 2

    def test_mixed_numeric_coded(self):
        from lattice_doe.utils import model_matrix_preview

        p, names = model_matrix_preview("~ C(g) + t", {"g": [0, 1, 2], "t": (0.0, 1.0)})
        assert p == 4
        assert "t" in names
    ...
    def test_continuous_only(self):
        ...
        p, _ = model_matrix_preview("~ x1 + x2", {"x1": (-1.0, 1.0), "x2": (0.0, 5.0)})
        assert p == 3
    ...
    def test_return_exact_continuous_is_provisional(self):
        ...
        p, _, exact = model_matrix_preview("~ C(I(x // 1))", {"x": (-2.0, 2.0)}, return_exact=True)
...
class TestSingularExchangeWarns:
    """Second-opinion review (2026-08-16): a singular moment matrix used to
    break out of the Fedorov exchange silently, returning the current design
    with no signal that optimization stopped early. Practically unreachable
    with the default jitter ridge, so reproduced with jitter=0 and an exactly
    ...
    def test_singular_break_emits_warning_and_returns_design(self):
        ...
        with pytest.warns(RuntimeWarning, match="Singular moment matrix"):
            idx = _fedorov_exchange_single(X, 8, criterion="I", max_iter=20, seed=0, jitter=0.0)
        assert len(idx) == 8  # the current design is still returned
