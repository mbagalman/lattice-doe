...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description=(
            ...
            "event-loop state); ``workers > 1`` is rejected with a 422. "
            "Uvicorn's own ``--workers`` flag scales the synchronous "
            "endpoints, but the async ``/jobs`` API keeps state in process "
            "memory: run a single worker (the shipped default) when using "
            "``/jobs``, or add sticky routing / a shared store.\n\n"
            "## Async jobs\n"
