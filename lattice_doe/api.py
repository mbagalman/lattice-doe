...
def find_optimal_design(
    ...
    # --- 2/3. Build candidate set & cached model matrix ---
    # Sizing and generation both live in build_search_candidate so that
    # interfaces constructing a scenario contrast up front can reproduce this
    # run's exact coding by calling it with the same design_opts (UX-48).
    def _announce_candidate_size(n: int) -> None:
        if _reporter is not None:
            _reporter.emit(
                Phase.GENERATING_CANDIDATES,
                message=f"Generating {n} candidate points",
...
def find_multiresponse_design(
    ...
    # 2. Candidate set — sized and generated by the shared helper so a
    # scenario contrast built up front can reproduce this coding (UX-48).
    def _announce_mr_candidate_size(n: int) -> None:
        if _reporter is not None:
            _reporter.emit(
                Phase.GENERATING_CANDIDATES,
                message=f"Generating {n} candidate points",
