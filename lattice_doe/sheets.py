...
def _get_client(credentials: Optional[str]) -> "gspread.Client":
    ...
    """
    if not _HAS_GSPREAD:
        raise ImportError(f"gspread is required for Google Sheets integration. {_INSTALL_HINT}")
...
def _parse_config_sheet(
    ...
    str,
    Dict[str, Any],
    Union[PowerContrastConfig, PowerR2Config, None],
    DesignOptions,
    ...
    if "formula" not in settings or not settings["formula"]:
        raise SheetsError("Config sheet [SETTINGS] is missing the required 'formula' key.")
    ...
    if idx_responses is None and ("power_mode" not in settings or not settings["power_mode"]):
        raise SheetsError("Config sheet [SETTINGS] is missing the required 'power_mode' key.")
    ...
    def _bool(key: str, default: bool) -> bool:
        ...
        if raw in ("false", "no", "0"):
            return False
        raise SheetsError(f"Config sheet [SETTINGS] key '{key}' must be true/false; got {raw!r}.")
    ...
    if idx_responses is None:
        if power_mode == "r2":
            ...
            sentinels_after_contrast = [j for j in (idx_factors,) if j > idx_contrast]
    ...
    if idx_responses is not None:
        ...
        for row in rows[responses_data_start:]:
            ...
            if r_mode == "contrast":
                ...
                try:
                    ...
                    raise SheetsError(f"{_resp_ctx}: non-numeric L_row or delta: {e}") from e
                ...
                if not r_r2_str:
                    raise SheetsError(f"{_resp_ctx}: r2 mode requires r2_target (col 9).")
                ...
                if not r_glm_baseline_str:
                    raise SheetsError(f"{_resp_ctx}: glm mode requires baseline (col 16).")
                try:
                    L_vals = [float(v.strip()) for v in r_L_str.split(",") if v.strip()]
                    d_vals = [float(v.strip()) for v in r_d_str.split(",") if v.strip()]
                    r_glm_baseline = float(r_glm_baseline_str)
                except ValueError as e:
                    raise SheetsError(f"{_resp_ctx}: non-numeric value: {e}") from e
                pcfg = build_power_cfg(
                    dict(
                        power_mode="glm",
                        L=[L_vals],
        ...
        if len(specs) < 2:
            raise SheetsError(
                f"[RESPONSES] section must define at least 2 responses; " f"found {len(specs)}."
            )
        try:
...
def _warn_about_legacy_output(
    ...
    if not rows or [str(c) for c in rows[0][:2]] != ["response", "worksheet"]:
        return
    stale = [str(r[1]) for r in rows[1:] if len(r) > 1 and str(r[1]).startswith("MM_")]
    warnings.warn(
    ...
    * **Buckets** — run-frequency buckets DataFrame.
    ...
    if _is_mr:
        ...
        summary_rows = [
            ...
            [
                "condition_number",
                float(diags["condition_number"]) if "condition_number" in diags else "",
            ],
    ...
    if result.get("model_matrices") is not None and report.get("compound_criterion"):
        _mm_taken: set = {ws.title for ws in spreadsheet.worksheets()} | {"ModelMatrixIndex"}
        _mm_index_rows = [list(_MM_INDEX_HEADER)]
        _written_ids: List[Any] = []
        for _rname, _rmm in result["model_matrices"].items():
            _title = "MM_" + safe_name_slug(_rname, _mm_taken, maxlen=80, prefix="MM_")
...
def create_sheet_template(
    ...
    * **Buckets** — empty; populated by ``sheets_run()``.
    ...
    if example not in _TEMPLATE_ROWS:
        raise ValueError(
            f"Unknown example {example!r}. Supported values: " f"{sorted(_TEMPLATE_ROWS.keys())}."
        )
    ...
    try:
        ...
        raise SheetsError(f"Failed to populate template spreadsheet: {e}") from e
...
def sheets_run(
    ...
    *,
    ...
    try:
        ...
        raise SheetsError(f"Design optimisation failed: {e}") from e
