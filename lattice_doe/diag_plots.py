...
def create_diagnostic_plots(
    ...
    if p > 1:
        ...
        if len(vif_df) > 0:
            ...
            ax_vif.axhline(y=5, color="orange", linestyle="--", alpha=0.5, label="Moderate (VIF=5)")
            ax_vif.axhline(y=10, color="red", linestyle="--", alpha=0.5, label="High (VIF=10)")
    ...
    colors = [
        "red" if lev > threshold_high else "orange" if lev > threshold_moderate else "blue"
        for lev in leverages
    ]

    ax_lev.scatter(run_numbers, leverages, c=colors, alpha=0.6, s=30)
    ax_lev.axhline(y=p / n, color="green", linestyle="-", alpha=0.5, label=f"Mean (p/n={p/n:.3f})")
    ax_lev.axhline(
